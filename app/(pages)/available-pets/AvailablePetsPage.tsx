...
const AvailablePetsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#fceed5] dark:bg-gray-950 pb-20">
      ...
      {/* Filters Section */}
      <section className="container mx-auto px-4 py-10 md:py-12">
        <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-xl p-6 md:p-8 border border-(--color-secondary-monYellow)/20">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <h2 className="text-2xl md:text-3xl font-bold text-(--color-primary-darkBlue)">
              Find Your Perfect Match
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 md:gap-8">
            ...
            <div className="space-y-2">
              ...
              <select
                value={typeFilter}
                onChange={(e) => setTypeFilter(e.target.value)}
                className="w-full rounded-xl border-2 border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-white px-4 py-3.5 focus:border-(--color-secondary-monYellow) focus:ring-2 focus:ring-(--color-secondary-monYellow)/30 transition-all"
              >
                <option value="All">All Types</option>
                <option value="Dog">Dog</option>
                <option value="Cat">Cat</option>
        ...
        {isLoading ? (
          <div className="flex flex-wrap justify-center gap-8">
            {[...Array(8)].map((_, index) => (
              <div
                key={index}
                className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden w-full max-w-[380px] sm:w-[calc(50%-1rem)] lg:w-[calc(33.333%-2rem)] animate-pulse"
              >
          ...
          <div className="flex flex-wrap justify-center gap-8">
            {filteredPets.map((pet) => (
              <div
                key={pet.id}
                className="group relative rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 bg-white dark:bg-gray-800 w-full max-w-[380px] sm:w-[calc(50%-1rem)] lg:w-[calc(33.333%-2rem)]"
              >
                <div className="relative h-64 overflow-hidden">
                  <Image
                    src={pet.mainImage}
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start">
                    <h3 className="text-2xl font-bold text-(--color-primary-darkBlue) dark:text-white">
                      {pet.name}
                    </h3>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {pet.traits.map((trait, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-(--color-secondary-monYellow)/20 text-(--color-primary-darkBlue) dark:text-white rounded-full text-xs font-medium"
                      >
                        {trait}
                      </span>
                    ))}
      ...
      <section className="px-4 py-16 text-center">
        <div className="container mx-auto max-w-4xl rounded-3xl bg-(--color-secondary-monYellow) dark:bg-(--color-neutral-0)/50 border border-transparent dark:border-(--color-card-border) py-12 px-8 transition-colors duration-300 shadow-lg">
          <h3 className="text-2xl md:text-3xl font-bold text-(--color-primary-darkBlue) dark:text-white">
            Ready to meet your new friend?
          </h3>
